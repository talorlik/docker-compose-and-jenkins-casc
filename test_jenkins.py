...
REGULAR_USER = "user"
REGULAR_USER_PASSWORD = os.getenv("JENKINS_USER_PASSWORD", "user123")
...
class TestJenkinsAuthentication:
    ...
    def test_user_login_success(self):
        """Test that regular user can successfully log in with configured password."""
        # Try to access the API with regular user credentials
        response = requests.get(
            f"{JENKINS_URL}/api/json",
            auth=HTTPBasicAuth(REGULAR_USER, REGULAR_USER_PASSWORD),
            timeout=10
        )
        
        assert response.status_code == 200, (
            f"User login failed. Status code: {response.status_code}"
        )
    ...
    def test_user_login_failure_with_wrong_password(self):
        """Test that user login fails with incorrect password."""
        response = requests.get(
            f"{JENKINS_URL}/api/json",
            auth=HTTPBasicAuth(REGULAR_USER, "wrong_password"),
            timeout=10
        )
...
class TestUserPermissions:
    ...
    def test_user_cannot_access_manage_jenkins(self):
        """Test that regular user cannot access Manage Jenkins (no administer permission)."""
        response = requests.get(
            f"{JENKINS_URL}/manage",
            auth=HTTPBasicAuth(REGULAR_USER, REGULAR_USER_PASSWORD),
            timeout=10
        ...
        assert response.status_code == 403, (
            "Regular user should not have access to Manage Jenkins page"
        )
    
    def test_user_cannot_access_script_console(self):
        """Test that regular user cannot access script console (no administer permission)."""
        response = requests.get(
            f"{JENKINS_URL}/script",
            auth=HTTPBasicAuth(REGULAR_USER, REGULAR_USER_PASSWORD),
            timeout=10
        ...
        assert response.status_code == 403, (
            "Regular user should not have access to script console"
        )
    
    def test_user_has_job_read_permission(self):
        """Test that user has Job/Read permission."""
...
<project>
  ...
  <buildWrappers/>
        ...
        # Now test if user can read the job
    ...
    def test_user_has_job_build_permission(self):
        """Test that user has Job/Build permission."""
...
<project>
  ...
  <buildWrappers/>
        ...
        # Get crumb for regular user
        user_crumb_response = requests.get(
            f"{JENKINS_URL}/crumbIssuer/api/json",
            auth=HTTPBasicAuth(REGULAR_USER, REGULAR_USER_PASSWORD),
            timeout=10
        )
        ...
        user_headers = {}
        if user_crumb_response.status_code == 200:
            user_crumb_data = user_crumb_response.json()
            user_crumb = user_crumb_data.get("crumb")
            user_crumb_field = user_crumb_data.get("crumbRequestField", "Jenkins-Crumb")
            user_headers[user_crumb_field] = user_crumb
        
        # Try to build the job as regular user
        ...
        assert build_response.status_code == 201, (
            f"User should have Job/Build permission. Status: {build_response.status_code}"
        )
    ...
    def test_user_has_job_cancel_permission(self):
        """Test that user has Job/Cancel permission."""
        # This is tested implicitly - if user can build, they should be able to cancel
        ...
        assert response.status_code == 200, (
            "User should be authenticated and have basic access"
        )
    
    def test_user_has_job_workspace_permission(self):
        """Test that user has Job/Workspace permission."""
...
<project>
  ...
  <buildWrappers/>
        ...
        # Try to access workspace as regular user
        ...
        assert workspace_response.status_code != 403, (
            "User should have Job/Workspace permission"
        )
    ...
    def test_user_has_view_read_permission(self):
        """Test that user has View/Read permission."""
        ...
        assert response.status_code == 200, (
            "User should have View/Read permission"
        )
        
        data = response.json()
        assert "views" in data or "jobs" in data, (
            "User should be able to see views"
        )
